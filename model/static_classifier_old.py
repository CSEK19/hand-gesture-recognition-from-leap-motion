...
class StaticHandPoseClassifier:
  def __init__(self, model_weight, scaler_weight=None):
    ...
    self.classes  = ['palm', 'fist', 'stop', 'left', 'right', 'up', 'down', 'rotate', 'thumb_in', 'negative']
...
# https://sci-hub.se/https://ieeexplore.ieee.org/document/8572153
  ...
  def feature_extraction(self, X):
    # X: [wrist, palm, thumb(1-4), ..., pinky(1-4), yaw, pitch, roll, handedness]
    X = np.array(X)
    joints = X[:-4].reshape(-1,3)
    palm_angles = X[-4:-1]  #yaw, pitch, roll
    # pairwise distance of wrist
    wrist, palm = joints[:2]
    fingertips = joints[[5,9,13,17,21]]

    # pairwise distances between palm and 5 fingertips
    pairwise_distances = pdist(np.concatenate((palm.reshape(1,-1), fingertips), axis=0), metric='euclidean')
    scaled_distances = self.scaler.transform(pairwise_distances.reshape(1,-1))[0]
    
    # angle between fingertip joints
    vectors = fingertips - palm
    fingertip_angles = np.array([angle(vectors[i], vectors[i+1]) for i in range(0,4)])


    return np.concatenate((scaled_distances, fingertip_angles, palm_angles), axis=0)